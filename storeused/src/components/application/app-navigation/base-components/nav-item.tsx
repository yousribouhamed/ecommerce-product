...
const styles = sortCx({
    root: "group relative flex w-full cursor-pointer items-center rounded-md bg-transparent outline-focus-ring transition duration-100 ease-linear select-none hover:bg-muted focus-visible:z-10 focus-visible:outline-2 focus-visible:outline-offset-2",
    rootSelected: "bg-muted hover:bg-muted/80",
});
...
export const NavItemBase = ({ current, type, badge, href, icon: Icon, children, truncate = true, onClick }: NavItemBaseProps) => {
    ...
    const labelElement = (
        <span
            className={cx(
                "flex-1 text-md font-semibold text-foreground transition-inherit-all group-hover:text-foreground/80",
                truncate && "truncate",
